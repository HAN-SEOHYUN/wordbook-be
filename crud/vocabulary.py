...
def create_or_update_word(
    ...
    - (date, english_word)가 고유 키로 중복되면 korean_meaning 및 updated_at을 업데이트합니다.
    """
    sql = f"""
    INSERT INTO {TABLE_NAME} (date, english_word, korean_meaning)
    VALUES (%s, %s, %s)
    ...
    try:
        with conn.cursor() as cursor:
            cursor.execute(sql, (word.date, word.english_word, word.korean_meaning))
            conn.commit()
            ...
            select_sql = f"""
            SELECT id, date, english_word, korean_meaning, created_at, updated_at
            FROM {TABLE_NAME}
...
def update_word(
    ...
    """
    단어 ID를 기반으로 korean_meaning 및 english_word를 업데이트합니다.
    """
    sql = f"""
    UPDATE {TABLE_NAME}
    SET 
